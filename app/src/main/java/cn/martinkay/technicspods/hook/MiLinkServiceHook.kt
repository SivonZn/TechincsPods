...
object MiLinkServiceHook : HookContext() {
    private const val TAG = "TechnicsPods-MiLink"
    ...
    private fun requestBluetoothStatus(reason: String, allowReconnect: Boolean = false) {
        ...
        ctx.sendBroadcast(Intent(TechnicsPodsAction.ACTION_REFRESH_STATUS).apply {
            putExtra(TechnicsPodsAction.EXTRA_ALLOW_RFCOMM_RECONNECT, allowReconnect)
    ...
    private fun isOppoPod(device: BluetoothDevice): Boolean {
        ...
        val result = TechnicsDeviceMatcher.isTechnicsName(name)
    ...
    private fun batteryValue(params: cn.martinkay.technicspods.utils.miuiStrongToast.data.PodParams?): Int {
