...
import cn.martinkay.technicspods.utils.FocusIslandUtil
import cn.martinkay.technicspods.utils.SystemApisUtils
import cn.martinkay.technicspods.utils.SystemApisUtils.cancelAsUser
import cn.martinkay.technicspods.utils.SystemApisUtils.notifyAsUser
...
object MiBluetoothToastHook : HookContext() {
    ...
    private const val CONNECTION_CHANNEL_ID = "technicspods_connection_notification"
    private const val CONNECTION_CHANNEL_NAME = "TechnicsPods"
    ...
    override fun onHook() {
        ...
        fun createPodsNotification(
            ...
            if (bluetoothDevice == null) {
                Log.e("TechnicsPods", "createPodsNotification: btDevice null")
                return
            }
            try {
                ...
                val pendingIntent = PendingIntent.getActivity(
                    context,
                    0,
                    Intent("chen.action.technicspods.show_pods_ui").apply {
                        setClassName("cn.martinkay.technicspods", "cn.martinkay.technicspods.PopupActivity")
                    },
                    PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
                )
...
//                    tickerPic = logo
                ...
                Log.e("TechnicsPods", "Failed to create Pod Notification", e)
            }
        }
        ...
        fun cancelNotification(bluetoothDevice: BluetoothDevice, context: Context) {
            try {
                ...
                Log.e("TechnicsPods", "Failed to cancel Pod Notification!", e)
            }
        }
            ...
            notificationSettings = NotificationSettings.fromPrefs(prefs)
                    ...
                    val broadcastReceiver = object : BroadcastReceiver() {
                        override fun onReceive(p0: Context?, p1: Intent?) {
                            if (p1?.action == "chen.action.technicspods.sendstrongtoast") {
                                val batteryParams = p1.getParcelableExtra(
                                    "batteryParams",
                                    BatteryParams::class.java
                                ) ?: return
                                FocusIslandUtil.showBatteryIsland(context, batteryParams)
                            } else if (p1?.action == "chen.action.technicspods.updatepodsnotification") {
                                ...
                                if (settings.showConnectionNotification && batteryParams != null) {
                                    createPodsNotification(
                                        ...
                                        p1.getBooleanExtra(TechnicsPodsAction.EXTRA_RFCOMM_CONNECTED, true)
                                    )
                                } else if (device != null) {
                                    cancelNotification(device, context)
                                }
                            } else if (p1?.action == "chen.action.technicspods.cancelpodsnotification") {
                                val device = p1.getParcelableExtra(
                                    "device",
                                    BluetoothDevice::class.java
                                ) ?: return
                                cancelNotification(device, context)
                            } else if (p1?.action == TechnicsPodsAction.ACTION_PODS_ANC_CHANGED) {
                                // 同步耳机实际 ANC 状态到本地缓存，确保下次循环切换时状态准确
                                localAncMode = p1.getIntExtra("status", 1)
                            } else if (p1?.action == TechnicsPodsAction.ACTION_ADAPTIVE_MODE_CHANGED) {
                            ...
                            } else if (p1?.action == TechnicsPodsAction.ACTION_NOTIFICATION_SETTINGS_CHANGED) {
                            ...
                            } else if (p1?.action == TechnicsPodsAction.ACTION_CYCLE_ANC) {
                                ...
                                Intent(TechnicsPodsAction.ACTION_ANC_SELECT).apply {
                                    setPackage("com.android.bluetooth")
                    ...
                    val intentFilter = IntentFilter("chen.action.technicspods.sendstrongtoast")
                    intentFilter.addAction("chen.action.technicspods.updatepodsnotification")
                    intentFilter.addAction("chen.action.technicspods.cancelpodsnotification")
                    intentFilter.addAction(TechnicsPodsAction.ACTION_CYCLE_ANC)
                    // 监听耳机实际 ANC 状态变更广播，保持 localAncMode 与 RfcommController 同步
                    intentFilter.addAction(TechnicsPodsAction.ACTION_PODS_ANC_CHANGED)
                    // 监听 Adaptive 模式开关状态变更广播，确保跨进程实时同步
                    intentFilter.addAction(TechnicsPodsAction.ACTION_ADAPTIVE_MODE_CHANGED)
                    intentFilter.addAction(TechnicsPodsAction.ACTION_NOTIFICATION_SETTINGS_CHANGED)
