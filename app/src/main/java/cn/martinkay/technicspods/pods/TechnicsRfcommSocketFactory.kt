...
/**
 * Creates and connects the Technics/Airoha SPP socket.
 */
object TechnicsRfcommSocketFactory {
    ...
    private fun connectViaUuids(device: BluetoothDevice, logTag: String): BluetoothSocket {
        ...
        throw connectException("Unable to connect Technics RFCOMM socket via UUID", failures)
    }
    ...
    private fun connectViaChannel(device: BluetoothDevice, logTag: String): BluetoothSocket {
        ...
        val channelSocket = try {
            ...
            failures += e
            throw connectException("Unable to create Technics RFCOMM socket via channel $RFCOMM_CHANNEL", failures)
        }

        return tryConnect(channelSocket, "channel $RFCOMM_CHANNEL", logTag, failures)
            ?: throw connectException("Unable to connect Technics RFCOMM socket via channel $RFCOMM_CHANNEL", failures)
    }
